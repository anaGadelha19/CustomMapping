function MappingBlock(mapDiv, timelineDiv) {
  ...
  if (mapDiv[0].mapping_map) {
    ...
    // Also clear Leaflet classes and internal references
    mapDiv.removeClass("leaflet-container").empty();
    delete mapDiv[0].mapping_map;
  }
  ...
  const groupedLayers = L.control
    .groupedLayers(baseMaps, groupedLayersGroups, groupedLayersOptions)
    .addTo(map);
              ...
              if (feature.properties) {
                ...
                if (!$.isEmptyObject(feature.properties)) {
                  ...
                  if (
                    ...
                    "string" === typeof feature.properties[labelKey]
                  ) {
                    $("<span>", { class: "group-type" })
                      .text(feature.properties[labelKey])
                      .appendTo(popup);
                  }
                  // Add the popup comment.
                  const commentKey = overlayData.property_key_comment;
                  if (
                    feature.properties[commentKey] &&
                    "string" === typeof feature.properties[commentKey]
                  ) {
                    $("<span>", { class: "group-value" })
                      .text(feature.properties[commentKey])
                      .appendTo(popup);
                  }
    ...
    timeline.on("change", function (e) {
      if ($.isNumeric(e.unique_id)) {
        ...
        const currentEventEnd =
          "undefined" === typeof currentEvent.end_date
            ? null
            : currentEvent.end_date.data.date_obj;
        ...
        if ($.isNumeric(mapData["timeline"]["fly_to"])) {
          map.flyToBounds(eventFeatures.getBounds(), {
            maxZoom: parseInt(mapData["timeline"]["fly_to"]),
          });
        } else {
          if (mapData["timeline"]["show_contemporaneous"]) {
            ...
            $.each(this.config.event_dict, function (index, event) {
              if ($.isNumeric(index) && index != currentEvent.unique_id) {
                ...
                const eventEnd =
                  "undefined" === typeof event.end_date
                    ? null
                    : event.end_date.data.date_obj;
                // For a timeline using intervals, a portion of this event
                // must fall within the interval of the current event.
                if (
                  currentEventEnd &&
                  eventStart <= currentEventEnd &&
                  eventEnd >= currentEventStart
                ) {
                ...
                if (
                  !currentEventEnd &&
                  currentEventStart.getTime() == eventStart.getTime()
                ) {
                  features.addLayer(featuresByResource[event.unique_id]);
                }
              }
...
$(document).on("click", ".mapping-show-group-item-features", function (e) {
  ...
  itemsBlock
    .find(".search-filters")
    .html(groupPopup.find(".mapping-search-filters-template").html());
